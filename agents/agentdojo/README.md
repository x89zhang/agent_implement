...
When the selected `case` includes an injection task, the adapter places that injection goal into every AgentDojo injection vector for the selected suite by default. Set `injection_enabled: false` to keep the injection task declared in the case and result metadata while running without injecting any payload; these control runs are excluded from the ASR denominator. Set `attack_template` to wrap the goal with an AgentDojo attack template without changing those locations. Set `custom_injection_text` to replace only the goal inside the template. `injection_vectors` is only an optional location filter. Use `custom_injections` only when you want fully custom per-vector payloads; it takes precedence over the other injection settings when injection is enabled.
